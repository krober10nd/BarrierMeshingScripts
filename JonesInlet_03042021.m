...
%% DESCJIPTION: Generate the Jones Inlet patch for insertion into the GSBv4 msh.
% AUTHOR: KEITH ROBERTS
% LAST UPDATE: MARCH 4, 2021

%% DECLARE PARAMETERS FOR MESHING

% triangle res at outer bbox 115m

% The barJIer representation (these are the endpoints of the crestline).
WEIR_STRUCT.X = [-73.57557155 ; -73.565727886	]; %
WEIR_STRUCT.Y = [ 40.59234367; 40.588733549];


JI = mean([ WEIR_STRUCT.Y, WEIR_STRUCT.X]); 
...
BSZ = 0.03;
BBOXES{1} = [JI(2)-BSZ JI(2)+BSZ; JI(1)-BSZ JI(1)+BSZ];

% 0.025 x 0.025 degree box around JI with 10-m min. resolution
BSZ = 0.025;
BBOXES{2} = [JI(2)-BSZ JI(2)+BSZ; JI(1)-BSZ JI(1)+BSZ];
...
H0        = [30,10];        % MINIMUM MESH RESOLUTION IN METERS
...
GRADES{1}     = [0.25 0 -inf    % Use a spatially vaJIable gradation rate overland.
    0.35 inf 0] ;
GRADES{2}     = [0.15 0 -inf    % Use a spatially vaJIable gradation rate overland.
    0.05 inf 0] ;
